...
import com.roncoo.pay.utils.WxSign;
...
public class ScanPayController extends BaseController {
     ...
     * @return
     */
    @RequestMapping("/initPay2")
    public String initPay2(Model model , HttpServletRequest httpServletRequest){
        Map<String , Object> paramMap = new HashMap<String , Object>();
        ...
        String payWayCode = getString_UrlDecode_UTF8("payWayCode"); // 支付方式编码 支付宝: ALIPAY  微信:WEIXIN
        paramMap.put("payWayCode",payWayCode);
        String payTypeCode=getString_UrlDecode_UTF8("payTypeCode"); //微信支付方式 jsapi
        paramMap.put("payTypeCode", payTypeCode);
        String openId=getString_UrlDecode_UTF8("openId");
        paramMap.put("openId", openId);
        System.out.println("initPay2-----"+openId);
        String orderIp = getString_UrlDecode_UTF8("orderIp"); // 下单IP
        paramMap.put("orderIp",orderIp);
        String orderDateStr = getString_UrlDecode_UTF8("orderDate"); // 订单日期
        paramMap.put("orderDate",orderDateStr);
        ...
        if (!MerchantApiUtil.isRightSign(paramMap,rpUserPayConfig.getPaySecret(),sign)){
            ...
            BigDecimal orderPrice = BigDecimal.valueOf(Double.valueOf(orderPriceStr));
            ScanPayResultVo scanPayResultVo = rpTradePaymentManagerService.initDirectScanPay(payKey, productName, orderNo, orderDate, orderTime, orderPrice, payWayCode, orderIp, orderPeriod, returnUrl
                    , notifyUrl, remark, field1, field2, field3, openId, payTypeCode);
             
//             String prepay_id = "prepay_id="+ scanPayResultVo.getCodeUrl();//这里得到预支付id
             

        	 Map<String,Object> map2 = scanPayResultVo.getMap();
             ...
             String paySign=WxSign.createSign(map3,WeixinConfigUtil.readConfig("apikey"));
             model.addAttribute("appid",map2.get("appid"));
             model.addAttribute("timeStamp",timeStamp);
             model.addAttribute("nonceStr",map2.get("nonce_str"));
             model.addAttribute("signType","MD5");
             model.addAttribute("paySign",paySign);
             model.addAttribute("prepay_id",map2.get("prepay_id"));
